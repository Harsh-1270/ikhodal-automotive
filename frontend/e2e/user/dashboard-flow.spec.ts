...
test.describe("Dashboard Flow", () => {
  ...
  test("services are displayed in grid view", async ({ page }) => {
    ...
    await expect(page.locator(".services-grid").first()).toBeVisible();
  });
  ...
  test("user can switch to list view", async ({ page }) => {
    ...
    await expect(page.locator(".services-list").first()).toBeVisible();
  });
  ...
  test("category filters work correctly", async ({ page }) => {
    ...
    const categoryBtn = page
      .getByRole("button", { name: /service packages/i })
      .first();
