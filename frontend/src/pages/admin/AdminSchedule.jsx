...
const AdminSchedule = () => {
  ...
  const isUnavailable = (date) => {
    return getOverridesForDate(date).some(
      (o) => o.overrideType === "UNAVAILABLE",
    );
  };
  ...
  const handleMarkAsAvailable = async () => {
    ...
    const slotOverrides = getOverridesForDate(selectedDate).filter(
      (o) => o.overrideType === "SLOT_BLOCKED",
    );
  ...
  const handleTimeSlotToggle = async (slot) => {
    ...
    if (slot.status === "BLOCKED") {
      ...
      if (result.success) {
        showToast(
          `Time slot ${formatTime(slot.start)} is now available!`,
          "success",
        );
        await fetchTimeSlots(selectedDate);
      } else {
      ...
      if (result.success) {
        showToast(
          `Time slot ${formatTime(slot.start)} marked as unavailable!`,
          "success",
        );
        await fetchTimeSlots(selectedDate);
      } else {
      ...
      {toast.show && (
        <div className={`adm-sch-toast ${toast.type}`}>
          <div className="adm-sch-toast-icon">
            {toast.type === "success" ? (
              <Icons.CheckCircle color="#ffffff" />
            ) : (
              <Icons.XCircle color="#ffffff" />
            )}
          </div>
          <span className="adm-sch-toast-message">{toast.message}</span>
          <button
            className="adm-sch-toast-close"
            onClick={() =>
              setToast({ show: false, message: "", type: "success" })
            }
          >
        ...
        <div className="adm-sch-page-header">
          <div className="adm-sch-header-left">
            <h1 className="adm-sch-page-title">
              <span className="adm-sch-title-icon">
                <Icons.Calendar />
              </span>
              Schedule Management
            </h1>
          ...
          <div className="adm-sch-calendar-section">
            <div className="adm-sch-calendar-card">
              ...
              <div className="adm-sch-days-header">
                {dayNames.map((day) => (
                  <div key={day} className="adm-sch-day-name">
                    {day}
                  </div>
                ))}
              </div>
              ...
              <div className="adm-sch-calendar-grid">
                {days.map((date, index) => {
                  ...
                  return (
                    ...
                    >
                      <span className="adm-sch-day-number">
                        {date.getDate()}
                      </span>
                      {status === "holiday" && (
                        <span className="adm-sch-day-label">
                          {isSunday(date) ? "Sunday" : "Holiday"}
                        </span>
                      )}
                      {status === "unavailable" && (
                        <span className="adm-sch-day-label">Closed</span>
                      )}
                    </div>
                  );
                })}
            ...
            <div className="adm-sch-controls-card">
              {selectedDate ? (
                <>
                  ...
                  <div className="adm-sch-date-actions">
                    ...
                    <div className="adm-sch-action-buttons">
                      ...
                      >
                        <span className="adm-sch-btn-icon">
                          <Icons.PartyPopper />
                        </span>
                        <span>
                          ...
                          {timeSlots.map((slot, idx) => {
                            ...
                            return (
                              <div
                                ...
                                onClick={() =>
                                  isClickable && handleTimeSlotToggle(slot)
                                }
                                style={{
                              ...
                              >
                                ...
                                <div className="adm-sch-slot-details">
                                  <div className="adm-sch-slot-time">
                                    {formatTime(slot.start)}
                                  </div>
                                  <div className="adm-sch-slot-duration">
                                    {formatTime(slot.start)} -{" "}
                                    {formatTime(slot.end)}
                                  </div>
                                </div>
                  ...
                  {(dateIsHoliday || dateIsUnavailable) && (
                    <div className="adm-sch-info-message">
                      <div className="adm-sch-info-icon">
                        <Icons.Info />
                      </div>
                ...
                <div className="adm-sch-no-date">
                  <div className="adm-sch-no-date-icon">
                    <Icons.Calendar color="#94a3b8" />
                  </div>
                  <h3>Select a Date</h3>
                  <p>
                    Click on a date in the calendar to manage its availability
                    settings
                  </p>
                </div>
              )}
